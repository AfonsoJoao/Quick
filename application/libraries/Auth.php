...
class Auth {
    ...
    private $permissaoView = 'corpo/corpoCadProduto'; // Recebe o nome da view correspondente à página informativa de usuário sem permissão de acesso
    private $loginView = 'corpo/corpoCadCliente'; // Recebe o nome da view correspondente à tela de login
    ...
    function CheckAuth($classe, $metodo) {
         ...
         */
        $array = array('ControllerProduto' => $classe, 'cadproduto' => $metodo);
        ...
        if (count($resultMetodos) == 0) {
            $data = array(
                'ControllerProduto' => $classe,
                'cadproduto' => $metodo,
                'identificacao' => $classe . 'ControllerProduto/cadproduto' . $metodo,
                'privado' => 1
            );
            $this->CI->db->insert('metodos', $data);
            redirect(base_url($classe . 'ControllerProduto/cadproduto' . $metodo), 'refresh');
        } else {
            /*
